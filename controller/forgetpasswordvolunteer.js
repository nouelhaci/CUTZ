...
const Otp = require('../model/volunteerotp')
...
const SendOtp = async (req, res) => {
    ...
    try {
        ...
        if (!volunteer) {
            return res.status(404).json({ message: `No volunteer with email ${req.body.email}` })
        }
...
const ResetPassword = async (req, res) => {
    try {
        ...
        bcrypt.hash(req.body.password, 10, async (err, hash) => {
            ...
            await Volunteer.findOneAndUpdate({ email: req.body.email }, { password: hash, confirmPassword: hash })
            res.status(200).json({
