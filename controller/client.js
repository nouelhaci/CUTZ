  ...
  try {
    ...
    if (user) {
      if (eventReservationClient) {
        const clientRecord = await ClientRecord.create({
          firstName: eventReservationClient.clientID?.firstName,
          lastName: eventReservationClient.clientID?.lastName,
          organizationName:
            eventReservationClient.eventID?.orgId?.organizationName,
          eventType: eventReservationClient.eventID?.eventType,
          location: eventReservationClient.eventID?.location,
          addresses: eventReservationClient.eventID?.addresses,
          familySize: eventReservationClient.clientID?.familySize,
          checkIn: eventReservationClient.checkIN,
          checkOut: eventReservationClient.checkOut,
          reservedTime: "",
          attendance: "",
        });
      }
